(ns netops.operation
  "OperationActor -- one network-operator operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Network Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Network Provisioning Governor
  (:govern) and the rollout phase gate (:decide) before anything
  commits to the SSoT."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [netops.advisor :as netadvisor]
...
(defn build
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (netadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (netadvisor/-advise advisor store request)]
            {:proposal p :audit [(netadvisor/trace request p)]})))
